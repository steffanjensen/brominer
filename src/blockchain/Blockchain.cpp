...
Blockchain::Blockchain(const Currency& currency, tx_memory_pool& tx_pool, ILogger& logger, bool blockchainIndexesEnabled) :
  ...
  m_upgradeDetectorv1(currency, m_blocks, CURRENT_BLOCK_MAJOR + 1, logger),
  m_upgradeDetectorv2(currency, m_blocks, CURRENT_BLOCK_MAJOR + 2, logger),
  m_upgradeDetectorv3(currency, m_blocks, CURRENT_BLOCK_MAJOR + 3, logger),
  m_upgradeDetectorv4(currency, m_blocks, NEXT_BLOCK_MAJOR_LIMIT, logger),
...
bool Blockchain::init(const std::string& config_folder, bool load_existing) {
  ...
  if (!m_upgradeDetectorv1.init() || !m_upgradeDetectorv2.init() || !m_upgradeDetectorv3.init() || !m_upgradeDetectorv4.init()) {
...
bool Blockchain::validate_miner_transaction(const Block& b, uint32_t height, size_t cumulativeBlockSize,
  ...
  if (minerReward > reward) {
    logger(WARNING, BRIGHT_YELLOW) << "Coinbase transaction spend too much money: " << m_currency.formatAmount(minerReward) <<
      ", block reward is " << m_currency.formatAmount(reward);
    //return false;
  } else if (minerReward < reward) {
    logger(WARNING, BRIGHT_YELLOW) << "Coinbase transaction doesn't use full amount of block reward: spent " <<
...
bool Blockchain::pushBlock(const Block& blockData, const std::vector<Transaction>& transactions, block_verification_context& bvc) {
  ...
  m_upgradeDetectorv1.blockPushed();
...
void Blockchain::popBlock() {
  ...
  m_upgradeDetectorv1.blockPopped();
